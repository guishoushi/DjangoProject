...
class XiaomiSport:
    ...
    def change_step(self):
        ...
        data = 'userid=%s&last_sync_data_time=%s&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json=%s' % (
            self.user_id, int(time.time()), data_json)
        headers = {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
...
def main(phone, password, number):
    # 最大运行出错次数
    fail_num = 3
    while 1:
        while fail_num > 0:
            try:
                # 执行一键修改步数
                return XiaomiSport(phone, password, number).one_click_change_step()
